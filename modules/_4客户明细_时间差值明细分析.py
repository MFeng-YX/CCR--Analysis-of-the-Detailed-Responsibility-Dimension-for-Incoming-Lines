...
def main(input_path, output_dir, date_prefix):
    try:
        ...
        # 创建第一个工作表的数据
        sheet1_data = df.groupby(['揽收网点名称', 'K码', '客户名称'])['入库前后'].value_counts().unstack(fill_value=0).reset_index()
        ...
        # 创建第二个工作表的数据
        sheet2_data = df.groupby(['揽收网点名称', 'K码', '客户名称'])['入库后进线-进线与入库时间差分布区间'].value_counts().unstack(fill_value=0).reset_index()
